...
import (
	...
	"github.com/TecXLab/libgochannel"
...
var (
	...
	fwriteEnqcount_FO = 1
	//
	ch1         = libgochannel.CreateQueue()
	chFWriter   = libgochannel.CreateQueue()
	chFWriterFO = libgochannel.CreateQueue()
	chFileCron  = libgochannel.CreateQueue()
	chSendProd  = libgochannel.CreateQueue()
	poolWrite7r = libgochannel.CreatePool()
	//
	time_Index   = 0
	op_Index     = 1
	hp_Index     = 2
...
func Dequeue_NSECD(m models.NewChartRespModel) {
	nDqCount++
	start := time.Now()
...
func Dequeue(m models.NewChartRespModel) {
	nDqCount++
	start := time.Now()
	SaveHistoryDeqNew(m)
	elapsed := time.Since(start)
...
func DequeueFile(m models.NewChartRespModel) {
	fwriteDqcount++
	start := time.Now()
...
func DequeueFileFO(m models.NewChartRespModel) {
	fwriteDqcount_FO++
	start := time.Now()
...
func FEnqueue(ch chan<- interface{}, mod libgochannel.ChanMod) {
	ch <- mod
}
...
func SaveHistory(response models.NewChartRespModel) {
	...
	FEnqueue(chFWriter, libgochannel.ChanMod{Model: response, Func: func(m any) {
		DequeueFile(m.(models.NewChartRespModel))
	}})
	...
	if response.ExchID == ExIds["nseeq"] {
		...
		FEnqueue(ch1, libgochannel.ChanMod{Model: response, Func: func(m any) {
			Dequeue(m.(models.NewChartRespModel))
		}})
		Add_CDS_Enqueue_NSEEQ_Count()
	} else if response.ExchID == ExIds["nsefo"] {
		// Enqueue(chFWriterFO, response)

		FEnqueue(chFWriterFO, libgochannel.ChanMod{Model: response, Func: func(m any) {
			DequeueFileFO(m.(models.NewChartRespModel))
		}})
		...
		// Enqueue(ch1, response)
		FEnqueue(ch1, libgochannel.ChanMod{Model: response, Func: func(m any) {
			Dequeue_NSECD(m.(models.NewChartRespModel))
		}})
...
func UpDownServiceNew(sPath string, nExID int16) error {
	...
	for i := range files {
		...
		if nExID == ExIds["nseeq"] {
			FEnqueue(ch1, libgochannel.ChanMod{Model: request, Func: func(m any) {
				Dequeue(m.(models.NewChartRespModel))
			}})
		} else if nExID == ExIds["nsefo"] {
			FEnqueue(chFWriterFO, libgochannel.ChanMod{Model: request, Func: func(m any) {
				DequeueFileFO(m.(models.NewChartRespModel))
			}})
...
func FileWriting4(contractDetails models.ContractDetails, folderName string, IsUpdate bool, objChartMapModel models.ChartMapModel, ExchId int16, DtLogDateTime string, arrChartM []models.ChartHLOCModel, date string) {

	if len(contractDetails.SFullName) == 0 || contractDetails.NToken <= 0 {
		return
	}

	csvString := ""
	for i := len(objChartMapModel.KeySlice) - 1; i >= 0; i-- {
		csvString += objChartMapModel.CSVMap[objChartMapModel.KeySlice[i]].CSVarr + "\n"
	}
	wg.Add(1)
	//* Uncomment
	// poolWriter <- libgochannel.ChanMod{
	// 	Model: []any{csvString, contractDetails, ExchId, folderName, date},
	// 	Func: func(objList any) {
	// 		var filePath = CreateDirectory(
	// 			objList.([]any)[1].(models.ContractDetails).SFullName,
	// 			objList.([]any)[2].(int16),
	// 			objList.([]any)[3].(string),
	// 		)
	...
	// 		wr.WriteString(objList.([]any)[0].(string))
	...
	for token := range Store {
		...
		for j := range Store[token] {
			...
			for TF := range Store[token][j].StoreMap {
				...
				FileWriting4(contractDetails, TF, true, *Store[token][j].StoreMap[TF], int16(j), inputDate, nil, inputDate)
...
func CLerAll(c *gin.Context) {
	...
	for key := range TimeframeMap {
		for i := range TimeframeMap[key].ExTFarr {
			for j := range TimeframeMap[key].ExTFarr[i] {
				for k := range TimeframeMap[key].ExTFarr[i][j] {
					TimeframeMap[key].ExTFarr[i][j][k] = 0
				}
			}
		}
	}
...
func InitAll() string {
	...
	if Env.STAGE == "local" {
		Env.VOLUMNE_PATH = "/home/cedric/Projects/Samples"
	}
...
func EnqueueCopyFolders(val any) {
	FEnqueue(chFileCron, libgochannel.ChanMod{
		Model: val,
		Func: func(a any) {
			CopyFolders(a)
		},
...
func EnqueueTFFFile(val any) {
	...
	objData := val.(models.GenerateTFFileModel)
...
func EnqueueFileChronInput(val any) {
	FEnqueue(chFileCron, libgochannel.ChanMod{Model: val, Func: func(a any) {
		GenerateTFFiles(a)
	}})
}
...
func EnqueueGetFileFromLocalHelper(val any) {
	FEnqueue(chSendProd, libgochannel.ChanMod{
		Model: val,
		Func: func(a any) {
			GetFileFromLocalHelper(a)
		},
	})
}
